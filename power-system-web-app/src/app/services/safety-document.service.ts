...
export class SafetyDocumentService implements IMultimediaService{
  ...
  downloadAttachment(documentId: number, filename: string): Observable<any> {
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${documentId}/attachments/${filename}`);
		return this.http.get(requestUrl, {responseType: 'blob'});
  }

  getAttachments(documentId: number): Observable<MultimediaAttachment[]> {
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${documentId}/attachments`);
  ...
  getSafetyDocumentDevices(id:number):Observable<Device[]>{
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${id}/devices`);
  ...
  getAllSafetyDocuments():Observable<SafetyDocument[]>{
    let requestUrl = environment.documentsServerURL.concat("safety-documents/all");
    return this.http.get<SafetyDocument[]>(requestUrl);
  }

  getAllMineSafetyDocuments(documentOwner?: string):Observable<SafetyDocument[]>{
   
    let requestUrl = environment.documentsServerURL.concat("safety-documents");
  ...
  getSafetyDocumentById(id:number):Observable<SafetyDocument>{
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${id}`);
    return this.http.get<SafetyDocument>(requestUrl);
  }
  ...
  updateSafetyDocument(safetyDocument: SafetyDocument):Observable<SafetyDocument>{
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${safetyDocument.id}`);
  ...
  updateSafetyDocumentChecklist(checklist: Checklist):Observable<Checklist>{
    ...
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${checklist.safetyDocumentId}/checklist`);
  ...
  getCrewForSafetyDocument(id:number):Observable<SafetyDocument>{
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${id}/crew`);
    return this.http.get<SafetyDocument>(requestUrl);
  }

  getStateChanges(wrId:number): Observable<StateChange[]> {
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${wrId}/state-changes`);
		return this.http.get<StateChange[]>(requestUrl);
  }

  approveSafetyDocument(wrId:number): Observable<SafetyDocument> {
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${wrId}/approve`);
		return this.http.put<SafetyDocument>(requestUrl, {});
  }

  
  denySafetyDocument(wrId:number): Observable<SafetyDocument> {
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${wrId}/deny`);
		return this.http.put<SafetyDocument>(requestUrl, {});
  }

  
  cancelSafetyDocument(wrId:number): Observable<SafetyDocument> {
    let requestUrl = environment.documentsServerURL.concat(`safety-documents/${wrId}/cancel`);
		return this.http.put<SafetyDocument>(requestUrl, {});
  }
