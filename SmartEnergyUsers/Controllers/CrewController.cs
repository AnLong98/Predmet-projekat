...
namespace SmartEnergy.Users.Controllers
{
    ...
    public class CrewController : ControllerBase
    {
        ...
        {
            return Ok(_crewService.GetAllAsync());
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> AddCrewAsync([FromBody] CrewDto newCrew)
        {
            try
            {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> UpdateCrewAsync(int id, [FromBody] CrewDto modifiedCrew)
        {
            try
            {
                CrewDto crew = await _crewService.UpdateAsync(modifiedCrew);
                return Ok(crew);
            }
            catch (UserNotFoundException unf)
            {
