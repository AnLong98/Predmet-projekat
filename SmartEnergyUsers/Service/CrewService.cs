...
namespace SmartEnergy.Users.Service
{
    public class CrewService : ICrewService
    {
        ...
        public CrewService(UsersDbContext dbContext, IMapper mapper)
        {
        ...
        {
            return _mapper.Map<CrewDto>(_dbContext.Crews.Include(x => x.CrewMembers).FirstOrDefault( x=> x.ID == id));
        }

        public async Task<List<CrewDto>> GetAllAsync()
        {
            return _mapper.Map<List<CrewDto>>(_dbContext.Crews.Include(x => x.CrewMembers).ToList());
        }
        ...
        public async Task<CrewDto> InsertAsync(CrewDto entity)
        {
        ...
        public async Task<CrewDto> UpdateAsync(CrewDto entity)
        {
            Crew modifiedCrew = _mapper.Map<Crew>(entity);
            Crew oldCrew = _dbContext.Crews.Include(x => x.CrewMembers).FirstOrDefault(x => x.ID == entity.ID);
