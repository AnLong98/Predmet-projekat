...
namespace SmartEnergy.Documents.Services
{
    public class MultimediaService : IMultimediaService
    {
        ...
        public MultimediaService(DocumentsDbContext dbContext, IMapper mapper, IConfiguration configuration, DaprClient dapr)
        {
        ...
        public async Task AttachUserAvatar(IFormFile formFile, int userId)
        {
            ...
            try
            {
                UserDto user = await _dapr.InvokeMethodAsync<UserDto>(HttpMethod.Get, "smartenergyusers", $"/api/users/{userId}");
        ...
        public async Task<FileStream> GetUserAvatarStreamAsync(int userId, string imageURL)
        {
            try
            {
                UserDto user = await _dapr.InvokeMethodAsync<UserDto>(HttpMethod.Get, "smartenergyusers", $"/api/users/{userId}");
