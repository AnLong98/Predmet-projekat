...
namespace SmartEnergy.Documents.Services
{
    public class WorkRequestService : IWorkRequestService
    {
        ...
        public WorkRequestService(DocumentsDbContext dbContext, IMapper mapper, IIncidentService incidentService,
            IAuthHelperService authHelperService, IMultimediaService multimediaService, DaprClient daprClient)
        {
        ...
        public async Task<WorkRequestDto> GetAsync(int id)
        {
            WorkRequest workRequest =  await _dbContext.WorkRequests.FindAsync(id);
        ...
        public async Task <List<WorkRequestDto>> GetAllAsync()
        {
            return _mapper.Map<List<WorkRequestDto>>(_dbContext.WorkRequests.ToList());
        }

        public async Task<List<DeviceDto>> GetWorkRequestDevicesAsync(int workRequestId)
        {
        ...
        public async Task<WorkRequestDto> InsertAsync(WorkRequestDto entity)
        {
            await ValidateWorkRequestAsync(entity);
            ...
            await _daprClient.InvokeMethodAsync(HttpMethod.Get, "smartenergyphysical", $"/api/devices/from-incident/{entity.IncidentID}/to-work-request/{workRequest.ID}");


            return _mapper.Map<WorkRequestDto>(workRequest);
        }

        public async Task<WorkRequestDto> UpdateAsync(WorkRequestDto entity)
        {
            await ValidateWorkRequestAsync(entity);
        ...
        private async Task ValidateWorkRequestAsync(WorkRequestDto entity)
        {
            ...
            if (entity.Street == null || entity.Street != "")
            {
                try
                {
                    LocationDto location = await _incidentService.GetIncidentLocationAsync(entity.IncidentID);
        ...
        public async Task<bool> IsCrewMemberHandlingWorkRequestAsync(int crewMemberId, int workRequestId)
        {
            ...
            CrewDto crew = await _daprClient.InvokeMethodAsync<CrewDto>(HttpMethod.Get, "smartenergyusers", $"/api/crews/{wr.Incident.CrewID}"); 

            if(crew.CrewMembers.FirstOrDefault(x => x.ID == crewMemberId) == null)
                return false;
